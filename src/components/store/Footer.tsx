    ...
    <footer className="bg-slate-950 text-slate-400 border-t border-slate-800">
      <div className="border-b border-slate-800/80 bg-slate-900/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="flex items-center gap-4 p-4 rounded-xl bg-emerald-500/10 text-emerald-400 flex items-center justify-center shrink-0">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="space-y-4">
            <Link href="/" className="flex items-center gap-2.5">
              ...
              <div className="text-base font-extrabold text-white tracking-tight">
                UNITED ARI LANKA <span className="text-brand-400 text-xs font-semibold">(PVT) LTD</span>
              </div>
            </Link>
            <p className="text-xs text-slate-400 leading-relaxed">
              Supplying professional builders, contractors, electricians, plumbers, and DIY homeowners with heavy-duty tools, equipment, and structural materials across Sri Lanka.
            </p>
            <div className="pt-2">
              <a
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="flex items-start gap-2.5">
              ...
              <span>{settings?.address || 'NO.66/3 AMANULLA ROAD, KATHANKUDY 06, SRILANKA'}</span>
            </div>
            <div className="flex items-center gap-2.5">
              <Phone className="w-4 h-4 text-brand-400 shrink-0" />
              <a href={`tel:${settings?.phone || '0704194147'}`} className="hover:text-white">
                {settings?.phone || '0704194147'}
              </a>
            </div>
            <div className="flex items-center gap-2.5">
              <Mail className="w-4 h-4 text-brand-400 shrink-0" />
              <a href={`mailto:${settings?.email || 'Unitedarilanka@gmail.com'}`} className="hover:text-white">
        ...
        <div className="border-t border-slate-800/80 mt-12 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
          <p>© {new Date().getFullYear()} {settings?.store_name || 'UNITED ARI LANKA (PVT) LTD'}. All Rights Reserved.</p>
        </div>
      </div>
    </footer>
  );
